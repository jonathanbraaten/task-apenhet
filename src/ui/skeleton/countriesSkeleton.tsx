...
export default function CountriesSkeleton({ limit = 5 }: { limit: number }) {
      ...
      <Wrapper>
        <ul id="country_list__grid">
          {skeletons.map((_, index) => (
            <li
              key={index}
              className="flex flex-col gap-3 px-5 pt-5 pb-10 border rounded-md hover:bg-gray-100 bg-gray-200 transition-colors duration-75"
            >
